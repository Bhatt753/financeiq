...
#  Decision engine (easy vs hard goal logic)
...
def give_advice(income, total_expenses, remaining, category_totals):
    ...
    if rent_percent > 30:
        ...
        print(f"     Ideal rent should be under 30% (₹{income * 0.30:,.2f}).")
    ...
    if ent_percent > 10:
        save_amount = entertainment - (income * 0.10)
    ...
    if shop_percent > 10:
        save_amount = shopping - (income * 0.10)
    ...
    if trans_percent > 15:
        save_amount = transport - (income * 0.15)
...
def run_decision_engine(goal, required_monthly, current_savings, gap, income, category_totals):
    ...
    if difficulty == "easy":
        ...
        print(f"     Just maintain your current spending habits.")
        print(f"     Tip: Put your savings in a separate account so")
        print(f"     you don't accidentally spend it.")

    elif difficulty == "medium":
        print(f"\n  ⚠️  GOAL DIFFICULTY: MEDIUM")
        print(f"     You need ₹{gap:,.2f} more per month.")
        print(f"     Small adjustments can close this gap.\n")

        # Suggest reducing the easiest categories first
        suggestions = []

        entertainment = category_totals.get("Entertainment", 0)
        if entertainment > 0:
            cut = min(entertainment * 0.20, gap)
            suggestions.append(("Entertainment", cut, "Reduce outings or subscriptions"))

        shopping = category_totals.get("Shopping", 0)
        if shopping > 0:
            cut = min(shopping * 0.20, gap)
            suggestions.append(("Shopping", cut, "Avoid impulse purchases"))

        food = category_totals.get("Food & Groceries", 0)
        if food > 0:
            cut = min(food * 0.10, gap)
            suggestions.append(("Food & Groceries", cut, "Cook at home more often"))

        if suggestions:
            print(f"     Here are small changes that can help:")
            for cat, cut, tip in suggestions:
                print(f"       • Reduce {cat} by ₹{cut:,.2f} → {tip}")
        else:
            print(f"     Try reducing any non-essential expense by 10-15%.")

    elif difficulty == "hard":
        print(f"\n  🚨 GOAL DIFFICULTY: HARD")
        print(f"     You need ₹{gap:,.2f} more per month.")
        print(f"     This requires serious changes.\n")

        # Find top 2 biggest expenses
        sorted_cats = sorted(category_totals.items(), key=lambda x: x[1], reverse=True)
        top_two = sorted_cats[:2]

        print(f"     Your biggest expenses are:")
        for cat, amount in top_two:
            percent = (amount / income) * 100
            suggested_cut = amount * 0.20
            print(f"       • {cat}: ₹{amount:,.2f} ({percent:.1f}% of income)")
            print(f"         → Cut by 20% to save ₹{suggested_cut:,.2f}/month")

        print(f"\n     Additional options:")
        print(f"       • Look for freelance or part-time income")
        print(f"       • Extend your goal timeline to reduce monthly pressure")
        extended_months = goal["months"] + 3
        new_required = goal["amount"] / extended_months
        print(f"         e.g. {extended_months} months → only ₹{new_required:,.2f}/month needed")
...
if __name__ == "__main__":
    ...
    total_expenses, remaining, category_totals = show_summary(profession, income, expenses)
    metrics = calculate_metrics(income, total_expenses, remaining, category_totals)
    show_metrics(metrics)
    give_advice(income, total_expenses, remaining, category_totals)
    goal                 = get_goal()
    show_goal_analysis(goal, remaining, income, category_totals)
    show_charts(profession, income, expenses, total_expenses, remaining, category_totals)
    def calculate_metrics(income, total_expenses, remaining, category_totals):
    metrics = {}

    # Core metrics
    metrics["savings_rate"]    = round((max(remaining, 0) / income) * 100, 2)
    metrics["expense_ratio"]   = round((total_expenses / income) * 100, 2)
    metrics["monthly_surplus"] = round(remaining, 2)

    # Category-wise % of income
    metrics["category_percent"] = {}
    for cat, amount in category_totals.items():
        metrics["category_percent"][cat] = round((amount / income) * 100, 2)
    ...
    if metrics["expense_ratio"] > 90:  score -= 30
    elif metrics["expense_ratio"] > 80: score -= 15

    for cat, pct in metrics["category_percent"].items():
        if cat == "Rent/Housing"     and pct > 30: score -= 10
        if cat == "Food & Groceries" and pct > 20: score -= 10
        if cat == "Entertainment"    and pct > 10: score -= 5
        if cat == "Shopping"         and pct > 10: score -= 5
...
def show_metrics(metrics):
    print("\n" + "=" * 50)
    print("        📊 YOUR FINANCIAL METRICS")
    print("=" * 50)
    print(f"  Savings Rate    : {metrics['savings_rate']}%")
    print(f"  Expense Ratio   : {metrics['expense_ratio']}%")
    print(f"  Monthly Surplus : ₹{metrics['monthly_surplus']:,.2f}")
    ...
    for cat, pct in metrics["category_percent"].items():
        ...
        status = "✅" if pct <= limit else "❌"
        print(f"    {status} {cat:<20} : {pct}% of income")
